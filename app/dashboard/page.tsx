      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Tracking portfolio engagement and visitor metrics
          </p>
        </div>
        <button
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="rounded-3xl border border-border bg-card/20 p-8 shadow-sm">
            <h2 className="text-xl font-bold text-foreground mb-6">
              Section Health & Improvement Advisor
            </h2>
            <div className="space-y-6">
              {stats.sections.map((sec) => (
                <div
                  key={sec.section}
                  className="flex flex-col md:flex-row md:items-center gap-4 p-4 rounded-xl bg-background border border-border shadow-sm"
                >
                  <div className="flex-1">
                    <h3 className="font-bold capitalize text-lg text-primary">
                      #{sec.section}
                    </h3>
                  ...
                  <div className="flex gap-6 text-sm">
                    ...
                    <div>
                      ...
                      <span className="font-medium text-foreground">
                        {(sec.avgDwellMs / 1000).toFixed(1)}s
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <section className="rounded-3xl border border-border bg-card/20 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {stats.ctas.map((cta) => (
                <div
                  key={cta.label}
                  className="flex items-center justify-between p-4 rounded-xl bg-background border border-border shadow-sm"
                >
                  <span className="font-medium text-foreground">
                    {cta.displayName}
                  </span>
              ...
              {stats.topPages.map((page) => (
                ...
                >
                  <a
                    href={page.path}
                    target="_blank"
                    className="text-sm text-muted-foreground hover:text-primary flex items-center gap-2 truncate max-w-[200px] transition-colors"
                  >
                    {page.path}
